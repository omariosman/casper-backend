...
qr_code_dict = {"123": 0, "456": 0}
def send_address(request):
    #Write your script here
    
    data = JSONParser().parse(request)
    qr_code = data["qrCode"]
    pub_address = data["publicAddress"]
    with open('E:\Blockchain\Casper\public_keys\omar_pb_key.txt', 'w') as f:
        f.write(pub_address)

    qr_code_list = ["123", "456"]
    
    if qr_code in qr_code_list and qr_code_dict[qr_code] == 0:
        qr_code_dict[qr_code] = 1
        ...
        if(qr_code in qr_code_list and qr_code_dict[qr_code] == 1):
            print("This QR code has already been used.")
            return HttpResponse("<h1>Invalid QR code</h1>")
        else:
